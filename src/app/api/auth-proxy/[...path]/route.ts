...
function getSessionToken(cookieStore: Awaited<ReturnType<typeof cookies>>): string | undefined {
  // Check all known Auth.js cookie name variants
  const names = [
  ...
  // Fallback: find any cookie ending with "session-token"
...
async function handler(
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/api/auth/${targetPath}`, fetchOptions)
